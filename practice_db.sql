...
-- left join
select c.customer_id, b.BookingID
from customers AS c
...
ALTER TABLE old_customers ADD COLUMN location VARCHAR(300);
...
VALUES (1, "Mahjabeen", 0778921456),
    (2, "Karim", 0789012345),
    (3, "Mohadisa", 0743423569),
    (4, "Latif", 0799078234);

ALTER TABLE current_customers ADD COLUMN location VARCHAR(300);
...
ALTER TABLE Orders
ADD customer_id INT,
ADD CONSTRAINT fk_orders FOREIGN KEY (customer_id) REFERENCES current_customers (customer_id)

ALTER TABLE orders DROP FOREIGN KEY fk_orders;

ALTER TABLE Orders
ADD CONSTRAINT fk_orders FOREIGN KEY (customer_id) REFERENCES current_customers (customer_id) On UPDATE CASCADE
...
CREATE TABLE new_orders SELECT * FROM Orders;

SELECT * FROM new_orders;

SHOW COLUMNS FROM new_orders;

-- copy table with and keep the constraints
CREATE TABLE copy_orders LIKE Orders;
SHOW COLUMNS FROM copy_orders;

Show DATABASES;

CREATE DATABASE orders;
-- copy tables between databases
CREATE TABLE orders.Orders SELECT * FROM little_lemmon.Orders;
